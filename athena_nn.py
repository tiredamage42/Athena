...
def get_average_untrained_accuracy(samples=100):
    # build normal model
    initializer, input, _, _, target, accuracy, _, _ = build_feedforward_model('DataNN', mnist.num_classes, mnist.img_res_flat)
...
def build_testNN ():
    with tf.variable_scope('TestNN'):
        ...
        # the input layer
        input_layer = tf.placeholder(tf.float32, [batch_dimension_size, mnist.img_res_flat])

        # the weights and biases of the hidden layer
        weights = tf.Variable(tf.zeros([mnist.img_res_flat, mnist.num_classes]), name="weights", trainable=False)
        biases = tf.Variable(tf.zeros([mnist.num_classes]), name="biases", trainable=False)
...
def run_training(num_iterations, batch_size, debug_frequency):
    ...
    for i in range(num_iterations):
        if i % debug_frequency == 0 or i == num_iterations - 1:
            ...
            gen_weights = gen_weights.reshape([-1, mnist.img_res_flat + 1, mnist.num_classes])
...
gen_weights = gen_weights.reshape([-1, mnist.img_res_flat + 1, mnist.num_classes])[0]
...
plot_9_imgs(x_test.reshape([-1, mnist.img_res, mnist.img_res]), pred, "Prediction", 'athena-gen-preds')
